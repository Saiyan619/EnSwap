...
export const useInitializePool = () => {
  ...
  const initializePool = async ({
    ...
    mintAddB,
    fee,
  }: initializePoolProp): Promise<{ tx: string; poolRent: number }> => {
    if (!publicKey || !wallet) {
    ...
    const poolRent = await connection.getMinimumBalanceForRentExemption(
      8 + 2 + 32 + 32 + 32 + 32 + 32 + 1
    );
    ...
    try {
      ...
      const tx = await program.methods
        .initializePool(new BN(feePercentToBps))
        .accounts({
          mintA: mintAddAKey,
          mintB: mintAddBKey,
          signer: publicKey,
      ...
      if (err.message.includes("already processed")) {
        console.warn(
          "Transaction was already processed - this might be a false error"
        );
