...
export default function WithdrawLiquidityPage() {
  ...
  useEffect(() => {
    const fetchLpBalance = async () => {
      ...
      try {
        ...
        const totalSupply =
          Number(lpMintInfo.supply) / Math.pow(10, lpMintInfo.decimals);
        ...
        const balance =
          Number(userLpAccount.amount) / Math.pow(10, lpMintInfo.decimals);
  ...
  useEffect(() => {
    ...
    const estimatedA =
      (lpAmount * pool.reserves.formattedBalanceA) / totalLpSupply;
    const estimatedB =
      (lpAmount * pool.reserves.formattedBalanceB) / totalLpSupply;
      ...
      <div className="max-w-xl mx-auto px-4 py-8">
        ...
        <div className="glass rounded-3xl p-6 space-y-6">
            ...
            <>
              <div>
                ...
                <div className="space-y-3">
                  <div className="glass-light rounded-2xl p-4 flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-foreground">
                        {Number(estimatedAmountA.toFixed(6))}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Min:{" "}
                        {(estimatedAmountA * (1 - slippage / 100)).toFixed(6)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="glass-light rounded-2xl p-4 flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-foreground">
                        {estimatedAmountB.toFixed(6)}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Min:{" "}
                        {(estimatedAmountB * (1 - slippage / 100)).toFixed(6)}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="glass-light rounded-2xl p-4 space-y-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-foreground">
                    {parseFloat(lpTokenAmount).toFixed(6)}
                  </span>
                </div>
                <div className="flex items-center justify-between text-sm">
          ...
          <Button
            ...
            disabled={
              !lpTokenAmount || parseFloat(lpTokenAmount) <= 0 || isPending
            }
