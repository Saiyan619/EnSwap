...
export default function AddLiquidityPage() {
  ...
  const isFullRange =
    Number(amount0) <= Number(currentPrice) * 0.5 &&
    Number(amount0) >= Number(currentPrice) * 1.5;
  ...
  const handleAddLiquidity = async () => {
    const firstLp =
      pool.reserves.formattedBalanceA <= 0 &&
      pool.reserves.formattedBalanceB <= 0;
    let expectedLp;
    let minLpTokens;
    if (firstLp) {
      ...
      const slippageFactor = BigInt(
        Math.floor((1 - slippagePercent / 100) * 10000)
      );
          ...
          <div>
            ...
            <div className="glass-light rounded-2xl p-4 mb-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-muted-foreground">
                  Balance: {pool?.reserves.formattedBalanceA}
                </span>
              </div>
            ...
            <div className="glass-light rounded-2xl p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-muted-foreground">Amount</span>
                <span className="text-sm text-muted-foreground">
                  Balance: {pool?.reserves.formattedBalanceB}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <input
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-4 max-w-4xl">
              {options.map((opt) => (
                ...
                >
                  <div
                    className={`glass-light rounded-2xl p-4 cursor-pointer transition-all ${
